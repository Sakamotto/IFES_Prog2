...
def main():
	...
	while True:		
		...
		movimentar(retangulo1, canvas, dx, dy, "pink")
		
		movimentar(retangulo2, canvas, da, db, "green")
		
		inte1 = tadretangulo.intersec(retangulo1, retangulo2)
		inte2 = tadretangulo.intersec(retangulo2, retangulo1)

		if inte1 != None:
			canvas.create_rectangle(inte1, fill="black", outline="black")
			canvas.update()
		elif inte2 != None:
			canvas.create_rectangle(inte2, fill="black", outline="black")
			canvas.update()
		#
		
		time.sleep(0.10)
		
		if retangulo2[1][1] >= h - 3:
			db *= -1
		if retangulo2[1][0] >= w - 3:
			da *= -1
		if retangulo2[0][1] == 0:
			db *= -1
		if retangulo2[0][0] == 0:
		...
		if retangulo1[1][1] >= h - 3:
			dy *= -1
		if retangulo1[1][0] >= w - 3:
			dx *= -1
		if retangulo1[0][1] == 0:
			dy *= -1
		if retangulo1[0][0] == 0:
